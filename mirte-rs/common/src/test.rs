...
use super::{float_eq, structs::colour::ColourEnum};

fn assert_point_eq(point1: Point, point2: Point) {
  assert!(float_eq(point1.x, point2.x));
  assert!(float_eq(point1.y, point2.y));
}

fn assert_vec_eq(vec1: Vector3, vec2: Vector3) {
  assert!(float_eq(vec1.x, vec2.x));
  assert!(float_eq(vec1.y, vec2.y));
}
...
fn compute_squared_length() {
  let dir = Vector3::new(10.0, -5.0);
  assert!(float_eq(dir.squared_length(), 125.0));
}

#[test]
fn compute_length() {
  let dir = Vector3::new(3.0, -4.0);
  assert!(float_eq(dir.length(), 5.0));
}
